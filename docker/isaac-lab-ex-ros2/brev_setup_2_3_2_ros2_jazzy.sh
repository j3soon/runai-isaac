...
# Setup runs for roughly 20 minutes and the VM is not usable until it reboots.
# Publish the current phase so an operator can tell "still working" from
# "broken" with one command, and keep a transcript for after the reboot.
...
# Pull before rebooting, but do NOT run `docker compose up -d` here: the
# container cannot start until the reboot loads the 580 kernel module, so an
# attempt now only fails and the pull is the only useful part of it.
#
# Measured on a g6e.xlarge (4 vCPU): running this pull concurrently with the
# driver install above does NOT help. Overlapping them stretched the install
# from ~12 to ~25 minutes and the pull from ~16 to ~30, for ~33 minutes total
# against ~30 sequential -- the DKMS build and the image decompression contend
# for the same few cores. Keep them sequential.
set_state "pulling-image"
sudo docker compose -f "$APP_DIR/compose.yaml" pull --quiet || \
  echo "[setup] image pull failed; the service will pull on next boot"
